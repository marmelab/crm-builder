...
| developer | opus | Implements + commits in worktree. Applies the **Ponytail** minimization ladder (full mode) automatically on every ticket, via an inline prompt directive. Also writes ADRs in `adr/` when the change introduces a structural decision. Never writes SQL migrations — deploy-time only. |
| simple-developer | sonnet | 1-file cosmetic OR 1 single-field change on an existing entity (schema + view + type + form + show) OR 1 list filter reusing existing components (no new custom React component). Applies the **Ponytail** ladder (full mode) automatically, via an inline prompt directive. No team, no review, never writes ADRs — SubagentStop hooks validate. POST-DEV runs if a migration was written. |
...
- Cold cache is expensive (~$0.17 for a "Hi!") — keep `enabledPlugins` minimal in `settings.json`. Two plugins are deliberately enabled, both declared via `extraKnownMarketplaces` and baked into the image at build time (`Dockerfile`, `claude plugin install` under `/home/developer/.claude/plugins/`) so the runtime PTY orchestrator never clones them nor stalls on an install prompt:
  - **Ponytail** (`ponytail@ponytail`, from `DietrichGebert/ponytail`): "lazy senior dev" code-minimization guidance (~540 always-on tokens/session, no MCP/agents/LSP). ⚠️ Its `SessionStart`/`UserPromptSubmit` hooks reach ONLY the main session (chat-orchestrator) — NOT the `Agent`-dispatched developer/simple-developer subagents (Claude Code issues #27661, #34692). So the plugin can NOT drive the dev agents. They get the Ponytail ladder from a minimal inline directive embedded directly in their prompts (`developer.md` / `simple-developer.md`) — no separate rule file. The plugin's value is the orchestrator-level guidance plus the manual skills `/ponytail`, `/ponytail-review`, `/ponytail-audit`, `/ponytail-debt` in an interactive session on the repo.
  - **Caveman** (`caveman@caveman`, from `JuliusBrussee/caveman`): output-token compressor (~75% prose reduction; preserves code, CLI commands, error strings and `<…>` markers verbatim). Same `SessionStart`/`UserPromptSubmit` hook mechanism → same subagent gap: it touches ONLY the orchestrator. **That gap is intentional here** — the dev/reviewer/merger subagents emit regex-parsed contract lines (`DONE:`/`FAILED:`/`APPROVED`/`REJECTED:`, plus the orchestrator's `<intent>`/`<session-title>` markers), so caveman is **deliberately NOT inlined** into any agent prompt (the opposite of Ponytail — caveman governs output prose, which must stay contract-compliant, not code). Pinned to `CAVEMAN_DEFAULT_MODE=lite` (`Dockerfile`) so the orchestrator's user-facing French chat stays readable (drops filler/hedging, keeps full sentences + structural tags); bump to `full` for harder compression. Manual skills `/caveman`, `/caveman-commit`, `/caveman-review`, `/caveman-stats`, `/caveman-compress` remain usable in an interactive session on the repo.
