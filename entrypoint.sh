...
# ── Resync harness config from the image on every boot ────────
# The harness (agents/skills/hooks/commands/rules/settings.json) ships in the
# AtomicCRM source and lives in the persistent crm-app volume at /app/.claude.
# Because the volume survives rebuilds, `make build-latest && make up` alone keeps
# the STALE first-boot copy — a new orchestrator/skill never reaches the running
# agent. Re-copy the image's authoritative .claude every boot. /app is a git repo
# and the merger runs `git reset --hard HEAD` in /app, which would revert an
# uncommitted resync to the stale committed tree — so commit it (idempotent:
# no-op when already in sync), same pattern as the CLAUDE.md/App.tsx commits below.
if [ -d /opt/atomic-crm-source/.claude ] && [ -d /app/.claude ]; then
  cp -a /opt/atomic-crm-source/.claude/. /app/.claude/
  chown -R developer:developer /app/.claude
  if [ -d /app/.git ]; then
    su developer -c '
      set -e
      cd /app
      BRANCH=$(git symbolic-ref --short HEAD 2>/dev/null || true)
      [ -z "$BRANCH" ] && exit 0  # detached HEAD / mid-rebase — skip
      [ -z "$(git status --porcelain .claude)" ] && exit 0  # already in sync
      git add .claude
      git -c user.name="Atomic CRM Builder" \
          -c user.email="builder@atomic-crm.local" \
          commit -m "chore: resync .claude harness config from image" --quiet
    ' || echo -e "${YELLOW}Could not resync/commit .claude (non-fatal)${NC}"
  fi
fi
...
# Project-level harness config (agents/skills/hooks/rules/settings.json) lives in
# /app/.claude — inside the persistent crm-app volume, NOT in CLAUDE_DIR — and is
# resynced from the image near the top of this script (see "Resync harness config
# from the image on every boot"). CLAUDE_DIR here holds only the login + the
# interactive-gate flags seeded above.
