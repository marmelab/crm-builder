...
**Superseded by the documentator redesign on branch `retakeDocumentator` (2026-05-05).**

The original phase 1 was a daily-cron, read-only synthesizer that wrote a structured "patterns ledger" to `/app/docs/learnings/patterns.md`. The cron, the loopback `POST /api/documentator/run` endpoint, the audit trail under `/app/docs/learnings/runs/`, and the `node-cron` dependency have all been removed. The agent is now triggered explicitly by the orchestrator on user request and produces runtime artifacts under `/home/developer/.claude/local/{rules,skills,hooks,agents}/` (see the updated `claudeConfig/.claude/agents/documentator.md` and the redesign commit). The ledger is preserved as an index pointing to those artifacts.
