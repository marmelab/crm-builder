...
description: Security review agent. Use after DEVELOPER implementation, in parallel with code-reviewer, test-validator, and ui-reviewer. Flags exploitable vulnerabilities, RLS issues, exposed secrets, and injection risks.
...
with other reviewers — you do not know their verdicts when you produce yours.
...
- Only `VITE_`-prefixed variables used client-side — they are public by definition
...
- Public API keys (if genuinely meant to be public)
  ...
  "issues": [
    {
      ...
      "file": "supabase/migrations/20250115_create_tickets.sql",
      "line": 18,
      "description": "SELECT policy uses USING (true) with no role restriction.",
      "fix": "Replace with USING (auth.role() = 'authenticated') or restrict by role."
    }
  ],
  "summary": "1 blocking issue. Must fix before merge."
}
```