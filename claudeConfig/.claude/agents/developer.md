...
Before building anything, walk the **Ponytail ladder** (`.claude/rules/ponytail.md`,
applied automatically — full mode) and stop at the first rung that satisfies the
...
### Rules
- **Ponytail (full mode) is always on** — apply the ladder in `.claude/rules/ponytail.md` on every ticket without being asked: native HTML/CSS and already-installed react-admin / shadcn components before any new component or dependency; deletion over addition; fewest files, shortest working diff. Adding a dependency for something the stack already covers is a blocking review finding. Never minimize away validation, security, accessibility, error handling, or required tests.
