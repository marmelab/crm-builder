#!/bin/bash
# SubagentStop hook — unit tests (app).
...
cd "$CLAUDE_PROJECT_DIR" || { echo "[$(date -Iseconds)] unit-app EXIT=0 cd_failed" >> "$LOG"; exit 0; }
...
if [ $EXIT_CODE -ne 0 ]; then
    ...
    echo "[$(date -Iseconds)] unit-app EXIT=2 npm_exit=$EXIT_CODE" >> "$LOG"
    exit 2
fi

echo "[$(date -Iseconds)] unit-app EXIT=0 OK" >> "$LOG"
exit 0
